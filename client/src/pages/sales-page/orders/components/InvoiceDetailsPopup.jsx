    ...
    <div className="flex-1 overflow-y-auto custom-scrollbar">
      ...
      <div className="p-6">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {activeTab === "info" && (
              <div className="space-y-6">
                ...
                {o.customRequirements && (
                  <div className="bg-amber-50/50 p-5 rounded-lg border border-amber-100/50 space-y-2">
            ...
            {activeTab === "products" && (
              <div className="space-y-6">
                {o.products.map((p, idx) => (
                  <div key={idx} className="bg-white rounded-lg overflow-hidden shadow-sm border border-slate-100 hover:shadow-md transition-all">
                    {/* Visual Comparison */}
                    ...
                    <div className="p-6">
                      ...
                      <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-slate-50 rounded-lg">
                        {[
          ...
          <div className="space-y-6">
            {hasPricing && (
              <div className="rounded-lg overflow-hidden bg-white border border-slate-100 shadow-sm">
...
function ProdItemRow({ item, onInspect, viewOnly }) {
  ...
  const needsKCS = (item.isPendingApproval || item.status === "Chờ nghiệm thu") && !viewOnly;
  ...
  return (
    <div className="p-4 border border-gray-100 rounded-lg bg-white shadow-xs space-y-3">
...
function ProductionProgressCard({ order, onInspect, onRedoRequest, productions, viewOnly }) {
  ...
  return (
    <div className="rounded-lg overflow-hidden bg-white/50 backdrop-blur-sm shadow-sm printer-hidden">
...
export default function InvoiceDetailsPopup({ invoiceId, isOpen, onClose, onStatusChanged, viewOnly }) {
  ...
  const handleSafeClose = () => {
    if (hasUnsavedChanges) {
      setConfirmConfig({
        title: "Xác nhận đóng",
        message: "Bạn có các thay đổi chưa được lưu. Bạn có chắc chắn muốn đóng không?",
        onConfirm: () => onClose()
      });
    } else {
      onClose();
                ...
                <>
                    ...
                    <button
                      ...
                      onClick={() => {
                        setConfirmConfig({
                          ...
                          message: "Xác nhận đơn hàng và chuyển sang trạng thái 'Chờ giao hàng'?",
                          onConfirm: () => handleUpdate("Chờ giao hàng")
                        });
                      }}
                    >
                  ...
                  {order.status === "Đang gia công" && (order.type === "Hàng mộc" || order.type === "Hàng khách đặt") && (
                    <button
                      ...
                      onClick={() => {
                        setConfirmConfig({
                          title: "Hoàn tất gia công?",
                          message: "Xác nhận sản phẩm đã hoàn thiện và sẵn sàng để giao?",
                          onConfirm: () => handleUpdate("Chờ giao hàng")
                        });
                      }}
                  ...
                  {/* Admin-only buttons removed for Sales view */}
                </>
              )}
                ...
                <button
                  ...
                  onClick={() => {
                    setConfirmConfig({
                      ...
                      message: `Xác nhận gửi YÊU CẦU HỦY cho đơn hàng ${order.code}? Chủ cửa hàng sẽ nhận thông báo và phê duyệt yêu cầu này.`,
                      onConfirm: () => handleUpdate("Chờ duyệt hủy", { cancelReason: "Nhân viên Sales yêu cầu hủy" })
                    });
                  }}
                >
                  <Ban size={16} /> {viewOnly ? "YÊU CẦU HỦY" : "HỦY"}
                </button>
              )}
            </div>
          </div>
        ...
        <div className="absolute inset-0 z-[100] flex items-center justify-center p-6 bg-black/40 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white w-full max-w-xl rounded-lg overflow-hidden modal-content transform animate-in zoom-in-95 duration-300 border border-gray-200">
            ...
            <div className="p-6 space-y-6 max-h-[80vh] overflow-y-auto custom-scrollbar">
              {/* Header Info */}
              <div className="bg-[var(--status-focus)]/50 rounded-lg p-5 grid grid-cols-2 gap-6 border border-[var(--brand-primary)]/10">
              ...
              {/* Bulk Apply Action */}
              <div className="bg-[var(--bg-main)] p-4 rounded-lg border border-[var(--grid-border)] flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
              ...
              <div className="space-y-3">
                ...
                <div className="border border-[var(--grid-border)] rounded-lg overflow-hidden shadow-xs bg-white">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <input
                    type="date"
                    className="w-full px-4 py-2.5 bg-[var(--bg-main)] border border-[var(--grid-border)] rounded-lg text-[13px] font-black text-gray-800 focus:ring-4 focus:ring-[var(--brand-primary)]/10 focus:border-[var(--brand-primary)] focus:bg-white outline-none transition-all"
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    className="w-full px-4 py-2 bg-[var(--bg-main)] border border-[var(--grid-border)] rounded-lg text-[13px] font-medium text-gray-700 focus:ring-4 focus:ring-[var(--brand-primary)]/10 focus:border-[var(--brand-primary)] focus:bg-white outline-none transition-all min-h-[42px] max-h-[80px] resize-none"
        ...
        <div className="absolute inset-0 z-[120] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-md animate-in fade-in">
          <div className="bg-white w-full max-w-md rounded-lg overflow-hidden animate-in zoom-in-95 duration-300 border border-gray-200">
            ...
            <div className="p-6 space-y-4">
              ...
              <textarea
                autoFocus
                className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-lg font-medium text-[13px] text-slate-800 focus:ring-4 focus:ring-rose-500/10 focus:border-rose-500 outline-none transition-all min-h-[120px]"
