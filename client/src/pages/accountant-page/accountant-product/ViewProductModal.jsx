...
const STATUS_COLOR = {
    ...
    "Ngừng kinh doanh":{ bg: "#F3F4F6", text: "#6B7280", border: "#D1D5DB" },
};
...
export default function ViewProductModal({ product, onClose }) {
    ...
    const statusColor = STATUS_COLOR[product.status] || STATUS_COLOR["Ngừng kinh doanh"];
                ...
                <div className="flex-1 overflow-y-auto">
                    ...
                    <div className="flex gap-0 border-b" style={{ borderColor: "var(--grid-border)" }}>
                        ...
                        <div className="flex-1 flex flex-col divide-y" style={{ borderColor: "var(--grid-border)" }}>
                            ...
                            {/* Hàng dưới: Tồn min / max */}
                            <div className="grid grid-cols-2 divide-x" style={{ borderColor: "var(--grid-border)" }}>
                                ...
                                <div className="p-3 flex items-center gap-3">
                                    <TrendingUp size={14} style={{ color: "var(--text-placeholder)" }} />
                                    <div>
                                        <p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: "var(--text-placeholder)" }}>Tồn tối đa</p>
                                        <p className="text-[15px] font-bold" style={{ color: "var(--text-main)" }}>{product.maxStock ?? "—"}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="px-6 py-2">
                        ...
                        <InfoRow icon={Tag}       label="Loại gỗ"       value={product.woodType} />
                        <InfoRow icon={Palette}   label="Màu sắc"       value={product.color} />
                        {dims.length > 0 && (
                            <InfoRow icon={Ruler} label="Kích thước (Dài × Rộng × Cao)"
                                value={`${dims.join(" × ")} cm`} />
                        )}
                        <InfoRow icon={MapPin}    label="Vị trí cất hàng" value={product.location} />
                        {product.details && (
