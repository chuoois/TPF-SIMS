...
import {
    X, Package, Tag, Layers, Palette, Ruler,
    BarChart2, CheckCircle, Hammer, Users,
    Image as ImageIcon, TrendingDown, ArrowDownToLine,
    Boxes, CalendarDays, ClipboardList, StickyNote, Info,
    ChevronDown, Hash,
} from "lucide-react";
...
const UNIT_STATUS_CONFIG = {
    AVAILABLE:        { label: "Sẵn sàng",     color: "#15803D", bg: "#F0FDF4", border: "#BBF7D0" },
    PROCESSING:       { label: "Đang gia công", color: "#7C3AED", bg: "#F5F3FF", border: "#DDD6FE" },
    PENDING_DELIVERY: { label: "Chờ giao",      color: "#1D4ED8", bg: "#EFF6FF", border: "#BFDBFE" },
    DEFECTIVE:        { label: "Hàng lỗi",      color: "#DC2626", bg: "#FEF2F2", border: "#FECACA" },
};

// Các trạng thái đang ở trong kho (hiển thị)
const IN_STOCK_STATUSES = ["AVAILABLE", "PROCESSING", "PENDING_DELIVERY", "DEFECTIVE"];
            ...
            <table className="w-full" style={{ backgroundColor: "#FAFAFE" }}>
                <thead>
                    <tr style={{ borderBottom: "1px solid #EDE9FE" }}>
                        ...
                        <th className="px-3 py-2 text-left text-[10px] font-bold uppercase tracking-wider" style={{ color: "#7C3AED" }}>Ghi chú</th>
                    </tr>
                </thead>
                <tbody>
                    {items.map((item, idx) => (
...
function ReceiptGroupCard({ receiptId, units, index }) {
    ...
    const visibleUnits = units.filter(u => IN_STOCK_STATUSES.includes(u.status));
    ...
    return (
        <div className="rounded-xl overflow-hidden" style={{ border: "1.5px solid var(--grid-border)" }}>
            {/* Header */}
            <button
            ...
            >
                ...
                <div className="flex items-center gap-3 shrink-0">
                    {importPrice != null && (
                        <span className="text-[12px] font-black" style={{ color: "#C2410C" }}>
            ...
            {!collapsed && (
                <div className="pb-4" style={{ backgroundColor: "#FAFAFE" }}>
                    {/* Mini stats bar */}
                    <div className="flex flex-wrap gap-1.5 px-4 py-3 border-b" style={{ borderColor: "var(--grid-border)" }}>
                        {[
                            { key: "available", label: "Sẵn sàng",     count: stats.available, ...UNIT_STATUS_CONFIG.AVAILABLE },
                            { key: "processing", label: "Đang gia công", count: stats.processing, ...UNIT_STATUS_CONFIG.PROCESSING },
                            { key: "delivering", label: "Chờ giao",      count: stats.delivering, ...UNIT_STATUS_CONFIG.PENDING_DELIVERY },
                            { key: "defective",  label: "Hàng lỗi",      count: stats.defective,  ...UNIT_STATUS_CONFIG.DEFECTIVE },
                        ].filter(s => s.count > 0).map(s => (
                            <span key={s.key} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold"
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-[12px]">
                            <thead>
                                <tr style={{ borderBottom: "1px solid var(--grid-border)", backgroundColor: "#F3F0FF" }}>
                                    <th className="px-4 py-2 text-left text-[9px] font-bold uppercase tracking-wider" style={{ color: "#6D28D9" }}>
                                        <span className="flex items-center gap-1"><Hash size={9} /> Mã định danh</span>
                                    </th>
                                    <th className="px-4 py-2 text-left text-[9px] font-bold uppercase tracking-wider" style={{ color: "#6D28D9" }}>
                                        <span className="flex items-center gap-1"><ArrowDownToLine size={9} /> Giá nhập</span>
                                    </th>
                                    <th className="px-4 py-2 text-left text-[9px] font-bold uppercase tracking-wider" style={{ color: "#6D28D9" }}>
                                        <span className="flex items-center gap-1"><CalendarDays size={9} /> Ngày nhập</span>
                                    </th>
                                    <th className="px-4 py-2 text-left text-[9px] font-bold uppercase tracking-wider" style={{ color: "#6D28D9" }}>
                                        Trạng thái
                                    </th>
                                    <th className="px-4 py-2 text-left text-[9px] font-bold uppercase tracking-wider" style={{ color: "#6D28D9" }}>
                                        <span className="flex items-center gap-1"><ClipboardList size={9} /> Phiếu nhập</span>
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                {visibleUnits.map((unit, idx) => {
                                    ...
                                    return (
                                        <tr key={unit.unitId}
                                            style={{
                                                borderBottom: idx < visibleUnits.length - 1 ? "1px solid #F3F0FF" : "none",
                                        ...
                                        >
                                            ...
                                            <td className="px-4 py-2.5">
                                                <span className="text-[12px] font-black" style={{ color: "#C2410C" }}>
                                                    {unit.importPrice != null ? fmtCurrency(unit.importPrice) : (importPrice != null ? fmtCurrency(importPrice) : "—")}
                                                </span>
                                            </td>
                                            {/* Ngày nhập */}
                                            <td className="px-4 py-2.5">
...
function UnitDetailTab({ product, lots }) {
        ...
        return (
            <div className="flex flex-col items-center justify-center py-16 gap-3">
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center"
                    style={{ backgroundColor: "var(--bg-main)", border: "2px dashed var(--grid-border)" }}>
                    <Boxes size={24} style={{ color: "var(--text-placeholder)" }} strokeWidth={1.5} />
                </div>
                <p className="text-[13px] font-medium" style={{ color: "var(--text-placeholder)" }}>
                    Chưa có dữ liệu đơn vị sản phẩm
                </p>
                <p className="text-[11px] text-center" style={{ color: "var(--text-placeholder)" }}>
    ...
    // Kiểm tra nhiều giá
    const prices = [...new Set(allUnits.map(u => u.importPrice).filter(p => p != null))];
    const hasMultiplePrices = prices.length > 1;

    return (
        <div className="px-5 py-4 space-y-4">
            {/* Tóm tắt tổng quan */}
            <div className="rounded-xl p-4" style={{ backgroundColor: "#F5F3FF", border: "1.5px solid #DDD6FE" }}>
                <p className="text-[10px] font-black uppercase tracking-widest mb-3 flex items-center gap-1.5" style={{ color: "#7C3AED" }}>
                    <Boxes size={11} /> Tổng quan đơn vị sản phẩm
                </p>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    <div className="flex flex-col gap-0.5">
                        <span className="text-[9px] font-bold uppercase tracking-widest" style={{ color: "#6D28D9" }}>Số phiếu nhập</span>
                        <span className="text-[20px] font-black" style={{ color: "#7C3AED" }}>{uniqueReceipts}</span>
                    </div>
                    <div className="flex flex-col gap-0.5">
                        <span className="text-[9px] font-bold uppercase tracking-widest" style={{ color: "#6D28D9" }}>Tổng đơn vị</span>
                        <span className="text-[20px] font-black" style={{ color: "#7C3AED" }}>{totalUnits}</span>
                    </div>
                    <div className="flex flex-col gap-0.5">
                        <span className="text-[9px] font-bold uppercase tracking-widest" style={{ color: "#6D28D9" }}>Sẵn sàng bán</span>
                        <span className="text-[20px] font-black" style={{ color: "#15803D" }}>{totalAvailable}</span>
                    </div>
                    <div className="flex flex-col gap-0.5">
                        <span className="text-[9px] font-bold uppercase tracking-widest" style={{ color: "#6D28D9" }}>Giá nhập</span>
                        {hasMultiplePrices ? (
                            <div>
                                <span className="text-[11px] font-black" style={{ color: "#C2410C" }}>
                                    {(Math.min(...prices) / 1_000_000).toFixed(0)}tr – {(Math.max(...prices) / 1_000_000).toFixed(0)}tr ₫
                                </span>
                                <span className="block text-[9px]" style={{ color: "#6D28D9" }}>(nhiều mức giá)</span>
                            </div>
                        ) : prices.length === 1 ? (
                            <span className="text-[13px] font-black" style={{ color: "#C2410C" }}>
                                {fmtMillions(prices[0])}
                            </span>
                        ) : (
                            <span className="text-[13px]" style={{ color: "var(--text-placeholder)" }}>—</span>
                        )}
                    </div>
                </div>
            </div>
                ...
                <div className="rounded-xl px-4 py-3 flex items-start gap-2.5"
                    ...
                    <div>
                        ...
                        <p className="text-[10px] mt-0.5" style={{ color: "#B45309" }}>
                            Giá thấp nhất: {fmtCurrency(Math.min(...prices))} · Cao nhất: {fmtCurrency(Math.max(...prices))}
            ...
            <div className="rounded-xl overflow-hidden" style={{ border: "1.5px solid var(--grid-border)" }}>
                <div className="px-4 py-2.5" style={{ backgroundColor: "var(--bg-main)", borderBottom: "1px solid var(--grid-border)" }}>
                    <p className="text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5" style={{ color: "var(--text-secondary)" }}>
                        <StickyNote size={10} /> Bảng tổng kết theo phiếu nhập
                    </p>
                </div>
                <table className="w-full bg-white text-[12px]">
                    <thead>
                        <tr style={{ borderBottom: "1px solid var(--grid-border)", backgroundColor: "var(--grid-header-bg)" }}>
                            <th className="px-4 py-2 text-left text-[9px] font-bold uppercase tracking-wider" style={{ color: "var(--text-placeholder)" }}>Phiếu nhập</th>
                            <th className="px-4 py-2 text-left text-[9px] font-bold uppercase tracking-wider" style={{ color: "var(--text-placeholder)" }}>Ngày nhập</th>
                            <th className="px-3 py-2 text-right text-[9px] font-bold uppercase tracking-wider" style={{ color: "var(--text-placeholder)" }}>Giá nhập</th>
                            <th className="px-3 py-2 text-center text-[9px] font-bold uppercase tracking-wider" style={{ color: "var(--text-placeholder)" }}>Trong kho</th>
                            <th className="px-3 py-2 text-right text-[9px] font-bold uppercase tracking-wider" style={{ color: "var(--text-placeholder)" }}>Giá trị tồn</th>
                        </tr>
                    </thead>
                    <tbody>
                        {receiptGroups.map(([receiptId, units], idx) => {
                            ...
                            const inStockCount = units.filter(u => IN_STOCK_STATUSES.includes(u.status)).length;
                            const totalVal = price != null ? inStockCount * price : null;
                            return (
                                <tr key={receiptId} style={{ borderBottom: "1px solid var(--grid-border)" }}>
                                    <td className="px-4 py-2.5">
                                        <span className="font-mono text-[10px] font-bold" style={{ color: "#7C3AED" }}>
                                            {receiptId === "__NO_RECEIPT__" ? "—" : receiptId}
                                        </span>
                                    </td>
                                    <td className="px-4 py-2.5" style={{ color: "var(--text-secondary)" }}>{fmtDate(date)}</td>
                                    <td className="px-3 py-2.5 text-right font-semibold" style={{ color: "#C2410C" }}>
                                        {price != null ? fmtCurrency(price) : "—"}
                                    </td>
                                    <td className="px-3 py-2.5 text-center">
                                        <span className="font-bold" style={{ color: inStockCount === 0 ? "#DC2626" : "var(--text-main)" }}>
                                            {inStockCount}
                                        </span>
                                    </td>
                                    <td className="px-3 py-2.5 text-right font-black"
                                        style={{ color: totalVal != null && totalVal > 0 ? "#15803D" : "var(--text-placeholder)" }}>
                                        {totalVal != null && totalVal > 0 ? fmtCurrency(totalVal) : "—"}
                                    </td>
                                </tr>
                            );
                        })}
                        ...
                        <tr style={{ backgroundColor: "#F5F3FF" }}>
                            <td className="px-4 py-3 text-[11px] font-black uppercase" style={{ color: "#7C3AED" }} colSpan={2}>
                                Tổng cộng
                            </td>
                            <td className="px-3 py-3" />
                            <td className="px-3 py-3 text-center font-black text-[13px]" style={{ color: "var(--text-main)" }}>
                                {allUnits.length}
                            </td>
                            <td className="px-3 py-3 text-right font-black text-[14px]" style={{ color: "#7C3AED" }}>
                                {fmtCurrency(
                ...
                <div className="flex-1 flex flex-col divide-y" style={{ divideColor: "var(--grid-border)" }}>
                    <div className="grid grid-cols-3 divide-x" style={{ borderColor: "var(--grid-border)" }}>
                        ...
                        <div className="p-4 flex flex-col gap-1">
                            ...
                            <span className="text-[26px] font-black leading-none" style={{ color: "#7C3AED" }}>
                                {product.lots?.reduce((s, l) => s + (l.units?.length || 0), 0) ?? 0}
                            </span>
                            <span className="text-[11px]" style={{ color: "var(--text-secondary)" }}>đơn vị đã nhập kho</span>
                        </div>
                    </div>
                    ...
                    {product.stockBreakdown && (
                        <div className="p-4 border-t" style={{ borderColor: "var(--grid-border)", backgroundColor: "#FAFAFA" }}>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {[
                                    { key: "available",  label: "Sẵn sàng",     count: product.stockBreakdown.available,  ...UNIT_STATUS_CONFIG.AVAILABLE },
                                    { key: "processing", label: "Đang gia công", count: product.stockBreakdown.processing, ...UNIT_STATUS_CONFIG.PROCESSING },
                                    { key: "delivering", label: "Chờ giao",      count: product.stockBreakdown.delivering, ...UNIT_STATUS_CONFIG.PENDING_DELIVERY },
                                    { key: "defective",  label: "Hàng lỗi",      count: product.stockBreakdown.defective,  ...UNIT_STATUS_CONFIG.DEFECTIVE },
                                ].filter(s => (s.count || 0) > 0).map(s => (
                                    <div key={s.key} className="flex flex-col items-center justify-center px-3 py-2 rounded-lg min-w-[62px]"
                                        style={{ backgroundColor: s.bg, border: `1px solid ${s.border}` }}>
                                        <span className="text-[18px] font-black leading-none" style={{ color: s.color }}>{s.count}</span>
                                        <span className="text-[9px] font-bold uppercase mt-1 text-center leading-tight" style={{ color: s.color }}>
                                            {s.label}
                                        </span>
                                    </div>
                                ))}
...
export default function ViewProductModal({ product, onClose }) {
    ...
    const [lots, setLots] = useState(() => product?.lots ? JSON.parse(JSON.stringify(product.lots)) : []);
    ...
    // Tính tổng units
    const allUnitCount = lots.reduce((sum, lot) => sum + (lot.units?.length || 0), 0);

    const TABS = [
        { id: "info", label: "Thông tin chung", icon: Info },
        { id: "units", label: `Chi tiết đơn vị${allUnitCount > 0 ? ` (${allUnitCount} đvị)` : ""}`, icon: Boxes },
    ];
        ...
        >
            <div className="w-full max-w-2xl bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden"
                ...
                <div className="px-6 py-5 shrink-0 relative" style={{ background: cfg.headerBg }}>
                    ...
                    <h2 className="text-[17px] font-black leading-snug pr-8" style={{ color: cfg.text }}>
                        {product.name}
                    </h2>
                    {product.bundleCode && (
                ...
                {/* ── Tab Navigation ── */}
                <div className="flex border-b shrink-0" style={{ borderColor: "var(--grid-border)" }}>
                    {TABS.map(tab => {
                ...
                <div className="flex-1 overflow-y-auto">
                    {activeTab === "info"
                        ? <GeneralInfoTab product={{ ...product, stockBreakdown: (() => {
                            if (!lots || lots.length === 0) return product.stockBreakdown;
                            const bd = { available: 0, processing: 0, delivering: 0, defective: 0 };
                            lots.forEach(lot => lot.units.forEach(u => {
                                if (IN_STOCK_STATUSES.includes(u.status)) {
                                    if (u.status === "AVAILABLE") bd.available++;
                                    else if (u.status === "PROCESSING") bd.processing++;
                                    else if (u.status === "PENDING_DELIVERY") bd.delivering++;
                                    else if (u.status === "DEFECTIVE") bd.defective++;
                                }
                            }));
                            return bd;
                        })(), stock: lots.length > 0
                            ? lots.reduce((s, l) => s + l.units.filter(u => IN_STOCK_STATUSES.includes(u.status)).length, 0)
                            : product.stock }} />
                        : <UnitDetailTab
                            product={product}
                            lots={lots}
