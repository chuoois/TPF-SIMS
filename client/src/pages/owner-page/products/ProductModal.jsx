...
const getStatusConfig = (status) => {
  switch (status) {
    case "Chưa định giá": return { bg: "#FEF2F2", text: "#DC2626", border: "#FECACA" };
    case "Hàng sẵn": return { bg: "#F0FDF4", text: "#15803D", border: "#BBF7D0" };
    case "Hàng mộc": return { bg: "#FFFBEB", text: "#D97706", border: "#FDE68A" };
    case "Hàng khách đặt": return { bg: "#EFF6FF", text: "#1D4ED8", border: "#BFDBFE" };
    case "Hết hàng": return { bg: "#FFF7ED", text: "#C2410C", border: "#FED7AA" };
    case "Quà tặng": return { bg: "#FAF5FF", text: "#7E22CE", border: "#E9D5FF" };
...
const Field = ({ label, value }) => (
  <div>
    <span className="text-[11px] text-[var(--text-secondary)] block mb-0.5">{label}</span>
    <span className="font-semibold text-[var(--text-main)] text-sm">{value || "—"}</span>
  </div>
);

/** Editable text input field */
const InputField = ({ label, value, onChange, placeholder, type = "text", postFix }) => (
  <div>
    ...
    <div className="relative">
      ...
      />
      {postFix && <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs font-bold text-slate-400">{postFix}</span>}
    </div>
  </div>
);
...
const PricingView = ({ item }) => {
  if (item.status === "Quà tặng") return (
    <div className="flex items-center justify-between">
      <span className="text-xs font-black text-purple-400 uppercase tracking-widest">Loại giá</span>
      <span className="text-xl font-black text-purple-700">MIỄN PHÍ</span>
    </div>
  );
  if (item.productType === "Hàng mộc") return (
    <div className="grid grid-cols-2 gap-3">
      <div>
        <span className="text-[10px] font-black text-amber-500 uppercase block mb-1">Giá bán mộc</span>
        <span className="text-lg font-black text-amber-700">{fmtCurrency(item.rawRetailPrice)}</span>
      </div>
      <div>
        <span className="text-[10px] font-black text-emerald-600 uppercase block mb-1">Giá hoàn thiện</span>
        <span className="text-lg font-black text-emerald-700">{fmtCurrency(item.finishedRetailPrice)}</span>
      </div>
    </div>
  );
  if (item.status === "Chưa định giá") return (
    <div className="text-center py-2">
      <span className="text-lg font-black text-[var(--status-error)] animate-pulse">CHỜ ĐỊNH GIÁ</span>
    </div>
  );
  return (
    <div className="flex items-center justify-between">
      <span className="text-xs font-black text-slate-400 uppercase tracking-widest">Giá niêm yết</span>
      <span className="text-xl font-black text-[var(--brand-primary)]">{fmtCurrency(item.retailPrice)}</span>
    </div>
  );
};
...
export default function ProductModal({
  ...
  useEffect(() => {
    if (product) {
      const latestLot = product.lots?.length > 0
        ? product.lots.reduce((latest, current) => new Date(current.importDate) > new Date(latest.importDate) ? current : latest, product.lots[0])
        : null;
      const initialCost = latestLot ? latestLot.importPrice : (product.costPrice || 0);
  ...
  const newestLot = product.lots?.length > 0
    ? product.lots.reduce((latest, current) => new Date(current.importDate) > new Date(latest.importDate) ? current : latest, product.lots[0])
    : null;

  const sc = getStatusConfig(product.status);
  const isWood = product.productType === "Hàng mộc";
  const canDelete = product.stock === 0 && (!product.lots || product.lots.length === 0);
    ...
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
      ...
      <div className="bg-white w-full max-w-[1000px] max-h-[90vh] rounded-lg flex flex-col relative animate-in zoom-in-95 duration-300 shadow-2xl overflow-hidden border border-slate-200">
        ...
        <div className="bg-white px-6 py-4 border-b border-slate-200 flex items-center justify-between shrink-0">
          ...
          <button onClick={onClose} className="p-1.5 hover:bg-slate-100 rounded-full transition text-slate-400 cursor-pointer">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
          <div className="flex gap-6">
            ...
            <div className="w-[160px] shrink-0 space-y-3">
              {product.img ? (
                <img src={product.img} alt={product.name} className="w-full aspect-square object-cover rounded-xl border" />
              ) : (
              ...
              {mode === "pricing" && (
                <div className="pt-2 space-y-2">
                  <div>
                    <span className="text-[11px] text-[var(--text-secondary)] block mb-0.5">Phân loại</span>
                    <span className="font-semibold text-[var(--text-main)] text-sm">{product.productType || "—"}</span>
                  </div>
                  <div>
                    <span className="text-[11px] text-[var(--text-secondary)] block mb-0.5">Tồn kho hiện tại</span>
                    <span className="font-semibold text-[var(--text-main)] text-sm">{product.stock} {product.unit}</span>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="flex-1 space-y-5">
              ...
              {mode === "edit" ? (
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2">
                    <InputField label="Tên sản phẩm *" value={form.name} onChange={set("name")} placeholder="Nhập tên sản phẩm" />
                  </div>
                  <InputField label="Mã sản phẩm *" value={form.code} onChange={set("code")} placeholder="VD: BBG-HS-..." />
                  <SelectField label="Danh mục" value={form.category} onChange={set("category")} options={CATEGORIES} placeholder="Chọn danh mục" />
                </div>
              ) : (
                <div>
                  <h1 className="text-lg font-bold text-[var(--text-main)] mb-0.5">{product.name}</h1>
              ...
              {mode === "edit" && (
                <div className="grid grid-cols-2 gap-4">
                  <SelectField label="Loại hàng" value={form.productType} onChange={set("productType")} options={["Hàng sẵn", "Hàng mộc", "Hàng khách đặt"]} />
                  <SelectField label="Chất liệu" value={form.material} onChange={set("material")} options={WOOD_TYPES} placeholder="Chọn chất liệu" />
                  <SelectField label="Màu sắc" value={form.color} onChange={set("color")} options={COLORS} placeholder="Chọn màu" />
                  <SelectField label="Đơn vị" value={form.unit} onChange={set("unit")} options={UNITS} placeholder="Chọn đơn vị" />
                  <div className="col-span-2">
                    <label className="block text-[11px] font-semibold text-[var(--text-secondary)] mb-1">Kích thước (D × R × C) cm</label>
                    <div className="flex items-center gap-2">
                      <input type="text" className="w-20 border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-[var(--brand-primary)]/30 text-center" placeholder="Dài" value={form.dimL} onChange={set("dimL")} />
                      <span className="text-slate-400 font-bold">×</span>
                      <input type="text" className="w-20 border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-[var(--brand-primary)]/30 text-center" placeholder="Rộng" value={form.dimW} onChange={set("dimW")} />
                      <span className="text-slate-400 font-bold">×</span>
                      <input type="text" className="w-20 border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-[var(--brand-primary)]/30 text-center" placeholder="Cao" value={form.dimH} onChange={set("dimH")} />
                    </div>
                  </div>
                  <div className="col-span-2">
                    <InputField label="Bảo hành (tháng)" type="number" value={form.warrantyMonths} onChange={set("warrantyMonths")} />
                  </div>
                  <div className="col-span-2">
                    <label className="block text-[11px] font-semibold text-[var(--text-secondary)] mb-1">Mô tả sản phẩm</label>
                    <textarea
                ...
                <>
                  <div className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
                    ...
                    {product.status !== "Chưa định giá" && (
                      <div>
                        <span className="text-[11px] text-[var(--text-secondary)] block mb-0.5">Bảo hành</span>
                        <span className="flex items-center gap-1 font-bold text-[var(--brand-primary)] text-sm">
                          <ShieldCheck size={13} /> {product.warrantyMonths || 12} tháng
                        </span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 space-y-3">
                    ...
                    <div className="border-t border-slate-200 pt-3 flex items-center justify-between">
                      <span className="text-xs text-slate-500">Tồn kho thực tế</span>
                      <span className="font-black text-[var(--text-main)]">
                        {product.productType === "Hàng khách đặt" ? "—" : `${product.stock} ${product.unit || "SP"}`}
                      </span>
                    </div>
                  </div>
                  ...
                  {product.description && (
                    <div>
                      <span className="text-[11px] font-semibold text-slate-400 uppercase block mb-1">Mô tả</span>
              ...
              {mode === "pricing" && (
                <>
                  ...
                  <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 space-y-4">
                    <div className="flex items-center justify-between">
                      <h3 className="text-xs font-bold uppercase text-slate-500 tracking-wider flex items-center gap-1.5">
                        <Wrench size={14} className="text-slate-400" /> 1. Chi phí cấu thành
                      </h3>
                      {newestLot && (
                        <span className="text-[10px] bg-white border border-slate-200 px-2 py-0.5 rounded text-slate-400 italic">
                          Lần nhập cuối: {new Date(newestLot.importDate).toLocaleDateString("vi-VN")}
                        </span>
                      )}
                    </div>
                      ...
                      <div className="col-span-2 grid grid-cols-3 gap-4 items-start">
                        <div className="col-span-1 flex flex-col gap-4">
                          ...
                          <div className="bg-slate-50/50 border border-slate-200 rounded-xl p-4 flex flex-col justify-center h-full">
                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Giá nhập gốc / Lô mới nhất</span>
                            <span className="text-2xl font-black text-[var(--text-main)]">{fmtCost(form.costPrice)}</span>
                          </div>
                        </div>

                        <div className="col-span-2 flex flex-col max-h-[180px] w-full">
                          <div className="flex items-center justify-between mb-3 shrink-0">
                            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Lịch sử lô hàng</span>
                            ...
                            {product.lots?.length > 0 ? (
                              [...product.lots]
                                ...
                                .map((lot, idx) => {
                                  ...
                                  return (
                                    <div key={lot.lotId || idx} className={`bg-white border ${isLatest ? 'border-emerald-200 shadow-sm shadow-emerald-500/10' : 'border-slate-100'} rounded-lg p-2.5 flex items-center justify-between transition-all hover:border-slate-300`}>
                                      <div className="flex items-center gap-3">
                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${isLatest ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-50 text-slate-400'}`}>
                                          <Package size={14} />
                                        </div>
                                        <div className="flex flex-col">
                                          <div className="flex items-center gap-2">
                                            ...
                                            {isLatest && <span className="text-[8px] bg-[var(--brand-primary)] text-white px-1.5 py-0.5 rounded uppercase font-black tracking-wider">Mới</span>}
                                          </div>
                                          <span className="text-[10px] font-medium text-slate-400 mt-0.5">
                                            Tồn: <span className="text-slate-600 font-bold">{lot.remainingQuantity ?? lot.initialQuantity ?? 0}</span> / {lot.initialQuantity ?? 0}
                                          </span>
                                        </div>
                                      </div>
                                      <span className={`text-xs font-black ${isLatest ? 'text-emerald-600' : 'text-slate-600'}`}>
                              ...
                              <div className="text-center py-4 bg-slate-50 rounded-lg border border-slate-100 text-xs text-slate-400 italic">Chưa có dữ liệu nhập hàng</div>
                            )}
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 space-y-4">
                    <h3 className="text-xs font-bold uppercase text-slate-500 tracking-wider flex items-center gap-1.5">
                      <Tag size={14} className="text-slate-400" /> 2. Quyết định giá niêm yết
                    </h3>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      ...
                      <InputField label="Biên lãi gộp (%)" value={formatNum(form.margin)} onChange={setNum("margin")} postFix="%" />
                      <InputField label="Hạn bảo hành" value={formatNum(form.warrantyMonths)} onChange={setNum("warrantyMonths")} postFix="Tháng" />
                    </div>
                    ...
                    <div className="p-4 bg-white border border-[var(--brand-primary)]/20 rounded-lg space-y-4 relative overflow-hidden">
                      ...
                      {isWood ? (
                        <div className="grid grid-cols-2 gap-5 mt-2">
                          <div>
                            <div className="flex justify-between items-end mb-1">
                              <label className="block text-xs font-bold text-amber-600 uppercase">Giá bán mộc</label>
                              <span
                                onClick={() => setForm(prev => ({ ...prev, rawRetailPrice: suggestedRaw }))}
                            ...
                            <div className="relative">
                              <input type="text" className="w-full border border-amber-200 bg-amber-50 rounded-lg px-3 py-2 text-lg font-black text-amber-700 outline-none focus:ring-2 focus:ring-amber-400/30" value={formatNum(form.rawRetailPrice)} onChange={setNum("rawRetailPrice")} />
                              <span className="absolute right-3 top-1/2 -translate-y-1/2 font-bold text-amber-400">₫</span>
                            </div>
                          </div>
                          <div>
                            <div className="flex justify-between items-end mb-1">
                              <label className="block text-xs font-bold text-emerald-600 uppercase">Giá hoàn thiện</label>
                              <span
                                onClick={() => setForm(prev => ({ ...prev, finishedRetailPrice: suggestedFinished }))}
                            ...
                            <div className="relative">
                              <input type="text" className="w-full border border-emerald-200 bg-emerald-50 rounded-lg px-3 py-2 text-lg font-black text-emerald-700 outline-none focus:ring-2 focus:ring-emerald-400/30" value={formatNum(form.finishedRetailPrice)} onChange={setNum("finishedRetailPrice")} />
                        ...
                        <div className="mt-2">
                          <div className="flex justify-between items-end mb-1">
                            <label className="block text-xs font-bold text-[var(--brand-primary)] uppercase">Giá bán niêm yết</label>
                            <span
                              onClick={() => setForm(prev => ({ ...prev, retailPrice: suggestedFinished }))}
                          ...
                          <div className="relative">
                            <input type="text" className="w-full border border-[var(--brand-primary)]/20 bg-[var(--brand-primary)]/5 rounded-lg px-4 py-3 text-2xl font-black text-[var(--brand-primary)] outline-none focus:ring-2 focus:ring-[var(--brand-primary)]/30" value={formatNum(form.retailPrice)} onChange={setNum("retailPrice")} />
                            <span className="absolute right-4 top-1/2 -translate-y-1/2 font-bold text-[var(--brand-primary)]/50">₫</span>
                          </div>
                        </div>
                      )}
                    </div>
