...
const RECENT_ACTIVITIES = [
  { id: 1, user: "Nguyễn Văn A", action: "Tạo mới đơn hàng", target: "ĐH-20240308-01", time: "10 phút trước", type: "order" },
  { id: 2, user: "Trần Thị B", action: "Cập nhật tồn kho", target: "Bàn ăn tròn xoay", time: "35 phút trước", type: "inventory" },
  { id: 3, user: "Lê Hoàng C", action: "Thêm thông tin khách hàng", target: "Công ty Vạn Phát", time: "1 giờ trước", type: "customer" },
  { id: 4, user: "Nguyễn Văn A", action: "Xác nhận báo giá", target: "BG-20240307-05", time: "2 giờ trước", type: "quote" },
  { id: 5, user: "Phạm D.", action: "Tạo phiếu nhập hàng", target: "Gỗ An Cường", time: "3 giờ trước", type: "supplier" },
  { id: 6, user: "Trần Thị B", action: "Sửa thông tin sản phẩm", target: "Ghế đôn sofa L", time: "Hôm qua", type: "product" },
];

const formatCurrency = (val) => new Intl.NumberFormat("vi-VN").format(val) + " ₫";

const MetricCard = ({ title, value, subtext, icon: Icon, trend, trendLabel, gradient, linkTo }) => {
  const CardContentBlock = (
    <Card className={cn("border-0 shadow-sm rounded-xl overflow-hidden relative bg-white group transition-shadow duration-300", linkTo ? "hover:shadow-md cursor-pointer hover:border-blue-200" : "hover:shadow-md")}>
      <CardContent className="p-4">
        <div className="flex justify-between items-center gap-4">
          <div className="flex-1">
            <p className="text-[12px] font-bold text-slate-500 uppercase tracking-wider mb-1">{title}</p>
            ...
            {subtext && <p className="text-[12px] text-slate-500 font-medium mt-1 truncate">{subtext}</p>}
          </div>
          <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center shrink-0 bg-gradient-to-br shadow-inner", gradient)}>
  ...
  return linkTo ? <Link to={linkTo} className="block relative hover:-translate-y-0.5 transition-transform">{CardContentBlock}</Link> : CardContentBlock;
};
      ...
      <div className="flex flex-col h-[calc(100vh-64px)] -m-6 p-6 md:p-8 space-y-8 overflow-y-auto bg-slate-50">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between shrink-0 gap-4">
          ...
          <div className="flex gap-3">
            <Button variant="outline" className="h-10 px-4 rounded-xl font-semibold shadow-sm border-slate-200 text-slate-700 bg-white" asChild>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 shrink-0">
          ...
          {/* Sắp hết hàng */}
          <Link to="/owner/products?tab=low_stock" className="bg-white rounded-xl p-4 shadow-sm border border-slate-200 flex items-center justify-between hover:border-orange-300 hover:shadow-md transition-all group">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-[11px] font-bold text-slate-500 mb-0.5 uppercase tracking-wider">Sắp hết hàng</p>
                <p className="text-[13px] font-medium text-slate-600">
                  <span className="font-bold text-orange-600 text-[15px] mr-1">{LOW_STOCK_PRODUCTS.length}</span>
                  sản phẩm
                </p>
              </div>
            </div>
            <ArrowRight size={16} className="text-slate-300 group-hover:text-orange-500 group-hover:translate-x-0.5 transition-transform" />
          </Link>

          {/* Cần duyệt gấp */}
          <Link to="/owner/orders?tab=pending" className="bg-white rounded-xl p-4 shadow-sm border border-slate-200 flex items-center justify-between hover:border-blue-300 hover:shadow-md transition-all group">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 rounded-lg bg-blue-50 flex items-center justify-center shrink-0 border border-blue-100/50">
                <Clock className="text-blue-600" size={20} />
              </div>
              <div>
                <p className="text-[11px] font-bold text-slate-500 mb-0.5 uppercase tracking-wider">Cần phê duyệt</p>
                <p className="text-[13px] font-medium text-slate-600">
                  <span className="font-bold text-blue-600 text-[15px] mr-1">{STATS.pendingApprovals}</span>
                  báo giá
                </p>
              </div>
            </div>
          ...
          {/* Nợ quá hạn */}
          <Link to="/owner/customers?tab=debt" className="bg-white rounded-xl p-4 shadow-sm border border-slate-200 flex items-center justify-between hover:border-rose-300 hover:shadow-md transition-all group">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-[11px] font-bold text-slate-500 mb-0.5 uppercase tracking-wider">Nợ quá hạn</p>
                <p className="text-[13px] font-medium text-slate-600">
                  <span className="font-bold text-rose-600 text-[15px] mr-1">{OVERDUE_CUSTOMERS.length}</span>
                  khách hàng
                </p>
              </div>
            </div>
            <ArrowRight size={16} className="text-slate-300 group-hover:text-rose-500 group-hover:translate-x-0.5 transition-transform" />
          </Link>
          ...
          <Card className="xl:col-span-3 border-0 shadow-sm rounded-2xl bg-white flex flex-col">
            <CardHeader className="py-5 px-6 border-b border-slate-50 flex flex-row items-center justify-between pb-4">
              <div className="space-y-1">
                <CardTitle className="text-[16px] font-bold text-slate-900">Biểu đồ doanh thu</CardTitle>
            ...
            <CardContent className="p-6 flex-1 min-h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={currentRevenueData} margin={{ top: 10, right: 20, left: 10, bottom: 20 }}>
                  <defs>
                  ...
                  <RechartsTooltip
                    ...
                    contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 10px 25px -5px rgba(0, 0, 0, 0.1), 0 8px 10px -6px rgba(0, 0, 0, 0.1)', fontSize: '13px', padding: '12px 16px', fontWeight: 500 }}
          ...
          <Card className="xl:col-span-2 border-0 shadow-sm rounded-2xl bg-white overflow-hidden flex flex-col">
            <CardHeader className="py-5 px-6 border-b border-slate-50 flex flex-row items-center justify-between bg-white z-10">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-[16px] font-bold text-slate-900">Top Sản Phẩm Bán Chạy Nhất</CardTitle>
              </div>
              <Button variant="ghost" size="sm" className="text-blue-600 font-semibold hover:text-blue-700 hover:bg-blue-50 h-8" asChild>
                <Link to="/owner/reports">Xem toàn bộ</Link>
              </Button>
            </CardHeader>
            <CardContent className="p-4 flex-1">
              <ResponsiveContainer width="100%" height={320}>
                <BarChart data={TOP_PRODUCTS} layout="vertical" margin={{ top: 10, right: 30, left: 40, bottom: 5 }}>
                      ...
                      if (active && payload && payload.length) {
                        ...
                        return (
                          <div className="bg-white p-3 rounded-xl shadow-lg border border-slate-100">
                            <p className="font-bold text-[13px] text-slate-800 mb-1">{data.name}</p>
                            <p className="text-[12px] text-slate-500 font-medium">Doanh thu: <span className="text-blue-600 font-bold">{formatCurrency(data.revenue)}</span></p>
                            <p className="text-[12px] text-slate-500 font-medium">Số lượng bán: <span className="text-slate-800 font-bold">{data.qty}</span> cái</p>
                          </div>
                        );
                      }
                      return null;
          ...
          <Card className="xl:col-span-1 border-0 shadow-sm rounded-2xl bg-white flex flex-col">
            <CardHeader className="py-5 px-6 border-b border-slate-50 flex flex-row items-center justify-between pb-4">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-[16px] font-bold text-slate-900">Nhật ký hoạt động</CardTitle>
              </div>
            </CardHeader>
            <CardContent className="p-0 flex-1">
              <div className="flex flex-col h-[350px] overflow-y-auto custom-scrollbar">
                {RECENT_ACTIVITIES.map((activity, index) => (
                  <div key={activity.id} className="px-6 py-4 flex items-start gap-3 border-b border-slate-50 hover:bg-slate-50/50 transition-colors last:border-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[13px] text-slate-600 font-medium leading-relaxed">
                        <span className="font-bold text-slate-800">{activity.user}</span> đã {activity.action.toLowerCase()} <span className="font-semibold text-slate-900 truncate">{activity.target}</span>
                      </p>
                      <p className="text-[11px] text-slate-400 font-medium mt-1 uppercase tracking-wider">{activity.time}</p>
                    </div>
                  </div>
                ))}
              </div>
