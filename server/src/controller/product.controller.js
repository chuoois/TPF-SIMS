...
class ProductController {
  ...
  async getAllProducts(req, res) {
    try {
      ...
      const isGiftQuery =
        is_gift === "true" || is_gift === "1" || is_gift === 1;
      if (isGiftQuery) {
      ...
      if (category_id) {
        const ids = String(category_id)
          ...
          .filter((n) => !isNaN(n));
        andConditions.push({
          fk_category_id: ids.length === 1 ? ids[0] : { [Op.in]: ids },
        });
      }
      if (color_id) {
        const ids = String(color_id)
          .split(",")
          .map(Number)
          .filter((n) => !isNaN(n));
        andConditions.push({
          fk_color_id: ids.length === 1 ? ids[0] : { [Op.in]: ids },
        });
      }
      if (material_id) {
        const ids = String(material_id)
          .split(",")
          .map(Number)
          .filter((n) => !isNaN(n));
        andConditions.push({
          fk_material_id: ids.length === 1 ? ids[0] : { [Op.in]: ids },
        });
      }
      if (room_id) {
        const ids = String(room_id)
          .split(",")
          .map(Number)
          .filter((n) => !isNaN(n));
        andConditions.push({
          fk_room_id: ids.length === 1 ? ids[0] : { [Op.in]: ids },
        });
      }
      ...
      if (!isGiftQuery) {
        if (sell_type == 1) {
          ...
          if (min_price)
            pricingInclude.where.final_price[Op.gte] = parseFloat(min_price);
          if (max_price) {
      ...
      const processedRows = rows.map((product) => {
        ...
        const pricing =
          p.pricings && p.pricings.length > 0 ? p.pricings[0] : null;
        ...
        if (isActuallyGift) {
          ...
          if (sell_type == 1) {
            ...
            original_price = pricing
              ? pricing.final_price || pricing.raw_price
              : 0;
  ...
  async getProductDetail(req, res) {
    try {
      ...
      if (coupon) {
        discount_percent = parseFloat(coupon.discount_percent);
        display_final_price =
          original_final_price * (1 - discount_percent / 100);
  ...
  /**
   * Xóa sản phẩm (Soft delete - chuyển trạng thái về 0)
   */
  async deleteProduct(req, res) {
    try {
      ...
      // Kiểm tra xem có đơn vị hàng nào gắn với sản phẩm này không
      const itemCount = await ProductItem.count({
      ...
      if (itemCount > 0) {
        return res.status(400).json({
          message:
            "Không thể xóa sản phẩm đã có lịch sử nhập kho hoặc đang có tồn kho.",
        });
      }

      // Chuyển trạng thái về 0 (Inactive/Deleted)
  ...
  async updateProduct(req, res) {
    ...
    try {
      ...
      // Xử lý cập nhật giá nếu có truyền thông tin giá
      if (
        cost_price !== undefined ||
        raw_price !== undefined ||
        final_price !== undefined
      ) {
        // Tìm record giá đang active
        const activePricing = await ProductPricing.findOne({
          where: { fk_product_id: id, status: 1 },
          transaction: t,
        });

        if (activePricing) {
          // Update record hiện tại
          await activePricing.update(
            {
              cost_price: cost_price !== undefined ? cost_price : activePricing.cost_price,
              raw_price: raw_price !== undefined ? raw_price : activePricing.raw_price,
              final_price: final_price !== undefined ? final_price : activePricing.final_price,
              modifiedate: new Date(),
            },
            { transaction: t }
          );
